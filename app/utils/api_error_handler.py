...
def handle_api_error(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        resource_id = f'con el ID {kwargs.get('id')}'
        ...
        try:
            ...
            if e.response.status_code == 404:
                logger.warning(f'Usuario no encontrado (404) en la base de datos externa {resource_id}. URL={e.request.url}')      
                raise HTTPException(status_code=404, detail=f'El Usuario {resource_id} no ha sido encontrado')
            else:
            ...
            logger.exception(f'Error interno inesperado en el endpoint: "{func.__name__}" {resource_id}: {e}')
